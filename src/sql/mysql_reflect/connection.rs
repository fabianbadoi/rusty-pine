...
impl LiveConnection {
    pub fn new(user:  &str, password: &str, host: &str, port: u16) -> Result<LiveConnection, PineError> {
...
impl Connection for LiveConnection {
    fn databases(&self) -> Result<Vec<String>, PineError> {
        ...
        let all_databases: Vec<_> = query_result
    ...
    fn tables(&self, db: &str) -> Result<Vec<String>, PineError> {
        let query_result = self.pool.prep_exec(format!("show tables from {}", db), ())?;
        let all_tables: Vec<_> = query_result
            .map(|row| from_row(row.unwrap()))
            .collect();
    ...
    fn show_create(&self, db: &str, table: &str) -> Result<String, PineError> {
        let query_result = self.pool.prep_exec(format!("show create table {}.{}", db, table), ())?;
